...
class DecoderLayers(nn.Module):
    def __init__(self, in_channels, out_channels, scale, target_res):
        super().__init__()
        ...
        self.fusion = ResnetBlockwithFAA(in_channels)


    def forward(self, x, skip=None, edge_feat=None):
        x = self.fusion(x, edge_feat)
        x = self.conv1(x)
        x = self.Upsampler(x)
        if skip is not None:
            x = x + skip
...
class FAA(nn.Module):
    ...
    def __init__(self, dim):
        super().__init__()
        self.param_free_norm = nn.InstanceNorm2d(dim, affine=False, track_running_stats=False)
        self.edge_conv = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=3, padding=1),
            nn.ReLU()
        )
        self.gate_conv = nn.Conv2d(dim*2, dim, kernel_size=3, padding=1)
        self.mlp_beta = nn.Conv2d(dim, dim, kernel_size=3, padding=1)

    def forward(self, x, edge):
        normalized = self.param_free_norm(x)
        edge_feat = self.edge_conv(edge)
        gamma = self.gate_conv(torch.cat([normalized, edge_feat], dim=1))
        out = normalized * (1 + gamma) + self.mlp_beta(edge_feat)
        return out
class ResnetBlockwithFAA(nn.Module):
    def __init__(self, dim):
        super(ResnetBlockwithFAA, self).__init__()
...
class Encoderlayer(nn.Module):
    def __init__(self,current_dim, next_dim, next_res, attn_res):
        ...
        if next_res in attn_res:
            ...
            self.attn = None
        self.fusion = ResnetBlockwithFAA(current_dim)


    def forward(self, x, mask, edge_feat):
       x = self.fusion(x, edge_feat)
...
class MutilScaleEncoder(nn.Module):
    ...
    def forward(self, x, mask, edge_feats):
        skips  = []
        x, mask = self.conv_first(x, mask)  # b, c, 128, 128
        skips.append(x)
        for i, layer in enumerate(self.enc_layers):
            x, mask = layer(x, mask, edge_feats[i])
...
class Decoder(nn.Module):
    def __init__(self, img_resolution, res, res_channel_map):
        ...
        for i in range(down_time):
            ...
            self.dec_conv.append(DecoderLayers(in_channels=current_dim, out_channels=target_dim, scale=2, target_res=target_res))

            current_res = target_res
            current_dim = target_dim

    def forward(self, x, skips, edge_feats):
        for i, block in enumerate(self.dec_conv):
            x = block(x, skips[len(self.dec_conv) - i - 1], edge_feats[i])
        return x
@ARCH_REGISTRY.register()
class DTFL(nn.Module):
    ...
    def forward(self, images_in, masks_in, edge, edge_en_outputs, edge_de_outputs):
        x, mask, skips = self.mutil_scale_encoder(images_in, masks_in, edge_de_outputs[::-1])
        x = self.decoder(x, skips, edge_en_outputs[::-1])
...
if __name__ == '__main__':
    ...
    img_resolution = 128
    batch = 1
    G = DTFL(img_resolution=128, img_channels=1).to(device)
    img = torch.randn(batch, 2, img_resolution, img_resolution).to(device)
    edge = torch.randn(batch, 1, img_resolution, img_resolution).to(device)
    mask = torch.randn(batch, 1, img_resolution, img_resolution).to(device)
    edge_en_outputs = [
        torch.randn(batch, 64, 64, 64).to(device),
        torch.randn(batch, 128, 32, 32).to(device),
        torch.randn(batch, 256, 16, 16).to(device),
        torch.randn(batch, 256, 8, 8).to(device)]
    edge_de_outputs = [
        torch.randn(batch, 256, 16, 16).to(device),
        torch.randn(batch, 128, 32, 32).to(device),
        torch.randn(batch, 64, 64, 64).to(device),
        torch.randn(batch, 32, 128, 128).to(device)]
    output = G(img, mask, edge, edge_en_outputs, edge_de_outputs)
    print(G)
    print(output.shape)
